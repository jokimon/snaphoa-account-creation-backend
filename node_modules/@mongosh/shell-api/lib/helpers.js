...
async function computeLegacyHexMD5(sp, str) {
    var _a;
    const digested = await ((_a = sp.computeLegacyHexMD5) === null || _a === void 0 ? void 0 : _a.call(sp, str));
    if (digested)
        return digested;
    let crypto;
    try {
        ...
        const collList = await Promise.all(colls.map(async (coll) => {
            ...
            for await (const chunk of (await chunksColl.find(chunksCollMatch)).sort({ min: 1 })) {
                if (chunksRes.length < 20 || verbose) {
                    ...
                    try {
                        coreUtilInspect !== null && coreUtilInspect !== void 0 ? coreUtilInspect : (coreUtilInspect = require('util').inspect);
                    }
                    catch (_a) {
                    }
                    if (coreUtilInspect) {
